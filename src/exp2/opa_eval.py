...
if __name__ == "__main__":
    ...
    obstacle_radii_torch = torch.from_numpy(obstacle_radii).to(
        torch.float32).to(DEVICE).view(-1, 1)
    obstacle_rot_radii_torch = torch.from_numpy(obstacle_rot_radii).to(
        torch.float32).to(DEVICE).view(-1, 1)
    object_radii_torch = obstacle_radii_torch
    object_rot_radii_torch = obstacle_rot_radii_torch
    object_radii = obstacle_radii
...
python exp1/unified_eval.py --collected_folder exp1/unified_saved_trials_obstacle/perturb_collection 
"""
