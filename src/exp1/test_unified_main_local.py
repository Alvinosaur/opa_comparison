...
def main():
    ...
    for exp_iter in range(num_exps):
        ...
        if exp_iter == 1:
            # perturb_pose_traj_orig = np.load("unified_perturb_pose_traj.npy")
            perturb_pose_traj = np.load("perturb_traj_iter_0_num_0.npy")
...
def view_trained_reward_traj():
    ...
    perturb_traj = np.load("perturb_traj_iter_0_num_0.npy")
    ...
    loaded_traj = np.load("test_unified_traj_1.npy")
    ax.plot3D(loaded_traj[:, 0], loaded_traj[:, 1], loaded_traj[:, 2],
              label="loaded", color="brown", linewidth=5)
