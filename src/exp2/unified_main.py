...
if __name__ == "__main__":
    ...
    for exp_iter in range(num_exps):
        ...
        while (not rospy.is_shutdown() and pose_error > pose_error_tol and
            ...
            #                          goal=goal_pose,
            #                          human_pose_euler=obstacle_pose,
            #                          context_dim=context_dim,
            #                          waypoints=TRAJ_LEN)
            #     traj = Trajectory(
            #         waypts=trajopt.optimize(
            #             context=obstacle_pose, reward_model=rm1),
            #         waypts_time=waypts_time)
            #     start_t = time.time()
            ...
            if kinova.need_update and not DEBUG:
                ...
                np.save(f"{save_folder}/perturb_traj_iter_{exp_iter}_num_{intervene_count}", kinova.perturb_pose_traj)
