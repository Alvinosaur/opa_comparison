...
class Trajectory(object):
	...
	def __init__(self, waypts, waypts_time):
		...
		if self.num_waypts > 1:
			self.timestep = (self.waypts_time[-1] -
			                 self.waypts_time[0]) / (self.num_waypts - 1)
	...
	def upsample(self, num_waypts):
		...
		"""
		assert num_waypts >= len(
			self.waypts), "Upsampling requires a larger number of waypoints to upsample to. Your number is smaller."
		...
		for i in range(num_waypts):
			if t >= self.waypts_time[-1]:
				...
				waypts_time[i] = t
				waypts[i, :] = curr_waypt + \
					((t - curr_waypt_idx * self.timestep) /
					 self.timestep) * (next_waypt - curr_waypt)
			t += timestep
	...
	def downsample(self, num_waypts):
		...
		"""
		assert num_waypts <= len(
			self.waypts), "Downsampling requires a smaller number of waypoints to downsample to. Your number is larger."
